...
export const StudentLessons: React.FC = () => {
  const { subjects, lessons, logActivity } = useApp();
  ...
  const handleOpenLesson = (lesson: Lesson) => {
    ...
    logActivity('view_lesson', `شاهد وتصفح الدرس: ${lesson.title}`, lesson.id);
  };
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">المواد الدراسية والمذكرات التعليمية</h2>
            <p className="text-xs text-slate-500">
              استعرض الدروس والمذكرات المقررة، شروحات الفيديو والروابط الإثرائية المضافة من المعلم
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {filteredLessons.length === 0 ? (
          <div className="col-span-full py-12 text-center bg-white rounded-2xl border border-slate-200 text-xs text-slate-400">
            لا توجد دروس متوفرة في هذا المقرر حاليًا
          </div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-[11px] text-slate-400">إشراف: {subj?.teacherName}</span>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <span>قراءة الدرس والمذكرات</span>
                  </button>
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full p-6 sm:p-8 border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-4">
              <div>
                <span className="text-[11px] font-bold px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-200">
                  {subjects.find((s) => s.id === activeLesson.subjectId)?.name}
                </span>
                <h2 className="text-xl font-black text-slate-900 mt-2">{activeLesson.title}</h2>
              </div>
              <button
            ...
            {activeLesson.summary && (
              <div className="p-4 rounded-xl bg-blue-50/70 border border-blue-100 text-xs text-blue-950 font-medium mb-5 leading-relaxed">
                <strong>خلاصة الدرس:</strong> {activeLesson.summary}
              </div>
            )}
            ...
            {activeLesson.externalLinks && activeLesson.externalLinks.length > 0 && (
              <div className="mb-6 space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {activeLesson.externalLinks.map((link, idx) => (
                    <a
                      key={idx}
                      href={link.url}
                      target="_blank"
                ...
                <div className="space-y-2">
                  {activeLesson.attachments.map((file, idx) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-amber-800 font-bold">تحميل المذكرة</span>
                    </div>
                  ))}
                </div>
              </div>
